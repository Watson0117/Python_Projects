...
def addToList(self):
    ...
    col_courses = self.txt_courses.get().strip()
        ...
        with conn:
            ...
            if count == 0 or count2 == 0 or count3 == 0: # if proposed changes are not already in the database, then proceed
                ...
                if response:
                    ...
                    with conn:
                        cursor = conn.cursor()
                        cursor.execute("""UPDATE tbl_StudentTracking SET col_phone = '{0}',col_email = '{1}',col_coursesl = '{2}' WHERE col_fullname = '{3}'""".format(var_phone,var_email,var_courses,var_value))
                        onClear(self)
                        conn.commit()
                else:

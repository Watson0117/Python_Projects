...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_StudentTracking (col_fname,col_lname,col_fullname,col_phone,col_email,col_courses) VALUES (?,?,?,?,?,?)""", ('John','Doe','John Doe','111-111-1111','jdoe@email.com','Math'))
            conn.commit()
    conn.close()
...
def onSelect(self,event):
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execute("""SELECT col_fname,col_lname,col_phone,col_email,col_courses FROM tbl_StudentTracking WHERE col_fullname = (?)""", [value])
        ...
        for data in varBody:
            ...
            self.txt_courses.delete(0,END)
            self.txt_courses.insert(0,data[4])
...
def addToList(self):
    ...
    var_courses = self.txt_courses.get().strip()
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and(len(var_email) > 0): # enforce the user to provide both names
        ...
        with conn:
            ...
            if chkName == 0: # if this is 0 then there is no existance of the fullname and we can add new data
                ...
                cursor.execute("""INSERT INTO tbl_StudentTracking (col_fname,col_lname,col_fullname,col_phone,col_email,col_courses) VALUES (?,?,?,?,?,?)""",(var_fname,var_lname,var_fullname,var_phone,var_email,var_courses))
...
def onUpdate(self):
    ...
    var_email = self.txt_email.get().strip()
    var_courses = self.txt_courses.get().strip()
    if (len(var_phone) > 0) and (len(var_email) > 0) and (len(var_courses) > 0): # ensure that there is data present
        conn = sqlite3.connect('db_StudentTracking.db')
        with conn:
            cur = conn.cursor()
            ...
            print(count2)
            cur.execute("""SELECT COUNT(col_courses) FROM tbl_StudentTracking WHERE col_courses = '{}'""".format(var_courses))
            ...
            if count == 0 or count2 == 0 or count3 == 0: # if proposed changes are not already in the database, then proceed
                ...
                if response:
                    ...
                    with conn:
                        cursor = conn.cursor()
                        cursor.execute("""UPDATE tbl_StudentTracking SET col_phone = '{0}',col_email = '{1}',col_courses = '{2}' WHERE col_fullname = '{3}'""".format(var_phone,var_email,var_courses,var_value))
                        onClear(self)
                        conn.commit()
                else:
                    messagebox.showinfo("Cancel request","No changes have been made to ({}).".format(var_value))
            else:
                messagebox.showinfo("No changes detected"," the ({}), ({}), and ({}) \nalready exist in the database for this name. \n\nYour update request has been cancelled.".format(var_phone, var_email, var_courses))
            onClear(self)
        conn.close()
    else:
